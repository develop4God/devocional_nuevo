// lib/widgets/encounter/encounter_key_verse_card.dart

import 'package:bible_reader_core/bible_reader_core.dart';
import 'package:devocional_nuevo/extensions/string_extensions.dart';
import 'package:devocional_nuevo/widgets/scripture/resolved_verse_text.dart';
import 'package:flutter/material.dart';

/// Key verse card for Encounters, rendered on the study's always-dark
/// (mood-colored) card surface. Colors are hardcoded for a dark background
/// rather than derived from Theme.of(context), since the surrounding
/// Material always paints dark regardless of the app's light/dark theme.
class EncounterKeyVerseCard extends StatelessWidget {
  final VerseRef keyVerse;
  final String? version;
  final EdgeInsetsGeometry? margin;
  final String? label;

  const EncounterKeyVerseCard({
    super.key,
    required this.keyVerse,
    this.version,
    this.margin,
    this.label,
  });

  @override
  Widget build(BuildContext context) {
    final theme = Theme.of(context);
    final colorScheme = theme.colorScheme;

    return Container(
      margin: margin ?? EdgeInsets.zero,
      decoration: BoxDecoration(
        gradient: LinearGradient(
          begin: Alignment.topLeft,
          end: Alignment.bottomRight,
          colors: [
            colorScheme.primary.withAlpha((0.40 * 255).round()),
            colorScheme.primary.withAlpha((0.18 * 255).round()),
            colorScheme.secondary.withAlpha((0.12 * 255).round()),
          ],
          stops: const [0.0, 0.6, 1.0],
        ),
        borderRadius: BorderRadius.circular(20),
        border: Border.all(
          color: colorScheme.primary.withAlpha((0.4 * 255).round()),
          width: 1.5,
        ),
        boxShadow: [
          BoxShadow(
            color: colorScheme.primary.withAlpha((0.25 * 255).round()),
            blurRadius: 20,
            offset: const Offset(0, 8),
            spreadRadius: -4,
          ),
          BoxShadow(
            color: Colors.black.withAlpha((0.08 * 255).round()),
            blurRadius: 40,
            offset: const Offset(0, 16),
            spreadRadius: -8,
          ),
        ],
      ),
      child: ClipRRect(
        borderRadius: BorderRadius.circular(20),
        child: Stack(
          children: [
            // Decorative background quote icon
            Positioned(
              top: -20,
              left: -10,
              child: Icon(
                Icons.format_quote_rounded,
                size: 140,
                color: colorScheme.primary.withValues(alpha: 0.03),
              ),
            ),

            Padding(
              padding: const EdgeInsets.all(32),
              child: Column(
                mainAxisSize: MainAxisSize.min,
                children: [
                  // Top Label
                  Container(
                    padding: const EdgeInsets.symmetric(
                      horizontal: 14,
                      vertical: 8,
                    ),
                    decoration: BoxDecoration(
                      color: Colors.white.withValues(alpha: 0.05),
                      borderRadius: BorderRadius.circular(20),
                      border: Border.all(
                        color: Colors.white.withValues(alpha: 0.1),
                      ),
                    ),
                    child: Row(
                      mainAxisSize: MainAxisSize.min,
                      children: [
                        Icon(
                          Icons.auto_stories_rounded,
                          size: 14,
                          color: colorScheme.primary.withValues(alpha: 0.6),
                        ),
                        const SizedBox(width: 10),
                        Text(
                          label ?? 'discovery.key_verse'.tr(),
                          style: theme.textTheme.labelSmall?.copyWith(
                            fontWeight: FontWeight.w800,
                            letterSpacing: 1.8,
                            color: Colors.white.withValues(alpha: 0.6),
                          ),
                        ),
                      ],
                    ),
                  ),

                  const SizedBox(height: 36),

                  // Verse Text
                  ResolvedVerseText(
                    reference: keyVerse.reference,
                    fallbackText: keyVerse.text,
                    quoted: true,
                    textAlign: TextAlign.center,
                    style: theme.textTheme.titleLarge?.copyWith(
                      fontWeight: FontWeight.w600,
                      color: Colors.white,
                    ),
                  ),

                  const SizedBox(height: 28),

                  // Reference and Version
                  Column(
                    children: [
                      Row(
                        mainAxisAlignment: MainAxisAlignment.center,
                        children: [
                          Container(
                            height: 1,
                            width: 24,
                            color: colorScheme.primary.withValues(alpha: 0.2),
                          ),
                          const SizedBox(width: 16),
                          Flexible(
                            child: Text(
                              keyVerse.reference.toUpperCase(),
                              style: theme.textTheme.titleSmall?.copyWith(
                                fontWeight: FontWeight.w900,
                                letterSpacing: 1.0,
                                color: colorScheme.primary,
                              ),
                              textAlign: TextAlign.center,
                            ),
                          ),
                          const SizedBox(width: 16),
                          Container(
                            height: 1,
                            width: 24,
                            color: colorScheme.primary.withValues(alpha: 0.2),
                          ),
                        ],
                      ),
                      if (version != null && version!.isNotEmpty) ...[
                        const SizedBox(height: 6),
                        Text(
                          version!,
                          style: theme.textTheme.labelSmall?.copyWith(
                            fontWeight: FontWeight.w500,
                            color: Colors.white.withValues(alpha: 0.4),
                          ),
                          textAlign: TextAlign.center,
                        ),
                      ],
                    ],
                  ),
                ],
              ),
            ),
          ],
        ),
      ),
    );
  }
}
