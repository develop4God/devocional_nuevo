...
class _DebugBulkAddSectionState extends State<DebugBulkAddSection> {
  ...
  Future<void> _addManyTestimonies(int count) async {
    ...
    final scaffold = ScaffoldMessenger.of(context);
    scaffold.showSnackBar(
      SnackBar(content: Text('Adding $count testimonies...')),
    );
    for (int i = 0; i < count; i++) {
      final text =
  ...
  Future<void> _fastAddManyPrayers(int count) async {
    final scaffold = ScaffoldMessenger.of(context);
    scaffold.showSnackBar(
      SnackBar(content: Text('Fast adding $count prayers...')),
    );
    try {
      ...
      scaffold.showSnackBar(
        SnackBar(content: Text('Error adding prayers: $e')),
      );
    }
  }
  ...
  Future<void> _fastAddManyThanksgivings(int count) async {
    ...
    try {
      ...
      scaffold.showSnackBar(
        SnackBar(
          content: Text(
            'Fast add $count thanksgivings — saved: ${saved.length}',
          ),
        ),
      );
  ...
  Future<void> _fastAddManyTestimonies(int count) async {
    ...
    try {
      ...
      for (int i = 0; i < count; i++) {
        ...
        final id = '${now.millisecondsSinceEpoch}_${rnd.nextInt(100000)}_$i';
        decoded.add(
          Testimony(id: id, text: text, createdDate: DateTime.now()).toJson(),
        );
      }
      ...
      scaffold.showSnackBar(
        SnackBar(
          content: Text('Fast add $count testimonies — saved: ${saved.length}'),
        ),
      );
  ...
  Future<void> _clearFast(
    ...
    try {
      ...
      scaffold.showSnackBar(
        SnackBar(
          content: Text('Cleared fast $label — remaining: ${filtered.length}'),
        ),
      );
    } catch (e) {
      debugPrint('Error clearing fast $label: $e');
      scaffold.showSnackBar(
        SnackBar(content: Text('Error clearing $label: $e')),
      );
    }
  }
