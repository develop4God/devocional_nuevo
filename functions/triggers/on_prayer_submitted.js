...
// Step 2: Enqueue moderation task to Cloud Tasks
//
...
const onPrayerSubmitted = onDocumentCreated(
  ...
  async (event) => {
    ...
    const piiService = new GeminiPiiMaskingService({
    ...
    try {
      ...
      // Continue to moderation anyway — prayer stays pending until manually resolved
    }

    // Step 2: Run moderation (in-process for simplicity; production deployments
    // can use Cloud Tasks queue by enqueuing the prayer ID and returning here).
    try {
