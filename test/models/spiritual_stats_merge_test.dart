...
void main() {
  group('SpiritualStats.merge()', () {
    test('Device A ahead — A has higher streak/reads, merged result ≥ A values',
        () {
    ...
    test('Device B ahead — B has higher streak/reads, merged result ≥ B values',
        () {
      ...
      expect(merged.readDevocionalIds.length, equals(25)); // union is 25 unique
      ...
      expect(merged.unlockedAchievements.length, equals(1)); // B's achievement
    });

    test('Disjoint read IDs — A has IDs 1–5, B has IDs 6–10, merged has all 10',
        () {
      // Device A: IDs 0–4
      final deviceA = SpiritualStats(
      ...
      expect(
          merged.readDevocionalIds.toSet().length, equals(10)); // No duplicates
    ...
    test(
      ...
      expect(merged.readDevocionalIds.toSet().length, equals(15)); // All unique
    ...
    test(
      ...
      expect(
        merged.unlockedAchievements.every((a) => a.isUnlocked),
        isTrue,
      );
    });
