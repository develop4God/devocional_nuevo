...
void main() {
  ...
  group('Backup — default getBackupOptions includes all categories', () {
    test('all 8 categories are enabled by default', () {
      ...
      expect(
        defaultOptions.containsKey(BackupKeys.completedEncounters),
        isTrue,
      );
  ...
  group('Backup payload — encounter completed IDs round-trip', () {
    ...
    test('encounter IDs written to prefs appear in backup payload', () async {
      ...
      final prefs = await SharedPreferences.getInstance();
      await prefs.setStringList('encounter_completed_ids', [
        'enc-001',
        'enc-002',
      ]);
      ...
      // Assert
      expect(
        payload[BackupKeys.completedEncounters],
  ...
  group('Restore — encounter completed IDs are written without restart', () {
    ...
    test(
      'after restore, isCompleted returns true for restored encounter IDs',
      () async {
        final backupPayload = {
  ...
  group('Backup payload — discovery progress round-trip', () {
    ...
    test(
      'discovery progress written by tracker appears in backup payload',
      () async {
  ...
  group('Backup payload — discovery favorites round-trip', () {
    ...
    test(
      'discovery favorites saved by service appear in backup payload',
      () async {
  ...
  group('Restore — discovery favorites readable without restart', () {
    ...
    test(
      'after restore, favorites for different languages are isolated',
      () async {
        final backupPayload = {
          BackupKeys.discoveryFavorites: {
            'discovery_favorite_ids_en': jsonEncode(['study-en-1']),
            'discovery_favorite_ids_es': jsonEncode([
              'study-es-1',
              'study-es-2',
            ]),
          },
  ...
  group('Restore safety — malformed or missing keys do not crash', () {
    ...
    test('restore with empty payload silently succeeds', () async {
      await expectLater(simulateRestoreBackupData({}), completes);
    });
    ...
    test(
      'discovery progress with non-prefixed keys are ignored during restore',
      () async {
